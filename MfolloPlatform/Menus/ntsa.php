...
// print_r($ResponseArray);
    ...
    private function menuLevel3() {
        ...
        $ExtraArray = split('*', $this->EXTRA);
        $this->strDISPLAY = "Are your details correct? IDNO($ExtraArray[0]]) and NAME($ExtraArray[1]]):\n1.Yes\n2.No^1^$this->EXTRA";
    }
    ...
    private function menuLevel5() {
        ...
        if ($this->EXTRA != "EXTRA") {
            $ExtraArray = split('*', $this->EXTRA);
            cacheAttribute("1", $ExtraArray[0]);
            cacheAttribute("2", $ExtraArray[1]);
            cacheAttribute("3", $ExtraArray[2]);

            $this->getProfileAttibutes();
        }
    }

    /* SECTION B - UTILITY FUNTIONS

     * This functions may be used across a number of menu Levels
    ...
    private function getProfileAttibutes() {
$profileID=$this->getProfileID();
        $query = "select attributeValue from serviceProfileAttributes where profileID=? and serviceID=?";
        $params = array('i', $profileID, "5");
        $result = D4M::createInstance(Props::$DBHOST, Props::$DBUSER, Props::$DBPASS, Props::$DBNAME)->DoSelect($query, $params);
        while ($row = mysqli_fetch_array($result)) {
            $this->ATTRIBUTES .= $row['attributeValue'] . "*";
        }
    }

    private function cacheAttribute($akID, $akValue) {
$profileID=$this->getProfileID();
        $values = array("profileID" => $profileID, "akID" => $akID, "attributeValue" => $akValue, "dateCreated" => date("h:i:sa"));
        $logged = D4M::createInstance(Props::$DBHOST, Props::$DBUSER, Props::$DBPASS, Props::$DBNAME)->DoInsert($table, $values);
