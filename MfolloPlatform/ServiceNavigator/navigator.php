...
//    }
    ...
    private function getServices() {
        ...
        $query = "select s.serviceID, s.URL, s.displayName, s.keyword  from services s join subscriptions sb on s.serviceID = sb.serviceID where sb.profileID = ? and s.status=? order by subscriptionID";
        $params = array('ii', &$profileID, &$status);
        $result = D4M::createInstance(Props::$DBHOST, Props::$DBUSER, Props::$DBPASS, Props::$DBNAME)->DoSelect($query, $params);
        while ($row = mysqli_fetch_array($result)) {
