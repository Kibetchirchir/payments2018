...
//            $this->sendSms($message);
    ...
    private function subscribe() {
        ...
        if ($serviceID != "") {
            ...
            $message = "You have been successfully subscribed for $serviceName. Dial *512# to access the service. Enjoy convenience at your finger tips.";
    ...
    private function checkIfProfileExists() {
        ...
        if ($profileID == "") {
            $table = "profiles";
            $values = array("MSISDN" => $this->MSISDN, "dateCreated" =>date("h:i:sa"), "netID"=> $this->NETID);
    ...
    private function sendSms($message)
    {
        ...
        if($this->SERVICE == '6011462000003259')
        {
            $sc='994';
            
        }  else if($this->SERVICE == '6011462000003257') {
            $sc='997';
        } else if ($this->SERVICE=='6011462000003258')
        {
            $sc='1009';
        }
        
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://localhost/MfolloPlatform/APIs/smsproxy.php?MSISDN=".$this->MSISDN."&MESSAGE=".rawurlencode($message)."&SOURCE=$sc");
